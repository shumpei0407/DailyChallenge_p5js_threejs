...
//赤・金
let pallete = ["#1C1C1C", "#EFBB24", "#66BAB7", "#CB1B45", "#4F4F48","#91989F"];
let sep = 5;
let rs;
...
function draw() {
  ...
  for (let y = -height/3; y < height; y += height / 100) {
    ...
    gradient.addColorStop(random(0.3,0.5), c2);
    ...
    for (let x = -100; x <= width+100; x+=10) {
      let yy = y-x/3 + map(noise(rs+y, x / 1000, frameCount / 100), 0, 1, -height / sep, height / sep);
      vertex(x, yy);
    }
